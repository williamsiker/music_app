  ...
  return (
    <motion.div key={index}
      ...
      <div className='w-275 min-w-[160px] flex items-center justify-center'>
        <img src={data.imageURL} alt='amogus' className='w-10 h-10 object-cover rounded-md min-w-[40px] shadow-md' referrerPolicy="no-referrer"/>
      </div>
      ...
      <div className='w-275 min-w-[160px] text-center flex items-center justify-center gap-6 realtive'>
        <p className='text-base text-textColor text-center'>{data.role}</p>
        {
          data._id !== user?.user._id && (
            ...
            hover:shadow-md' onClick={() => setIsUserRoleUpdate(true)}>
        ...
        {isUserRoleUpdate && (
          ...
          className='absolute z-10 top-6 right-4 p-4 flex items-start flex-col gap-4 bg-white
          shadow-xl rounded-md'>
            ...
            <div className='flex items-center gap-4'>
              <motion.button 
              whileTap={{scale:0.75}} 
              className='outline-none border-none text-sm px-4 py1 rounded-md bg-blue-200 
              text-black hover:shadow-md'
              onClick={() => updateUserRole(data._id, data.role === "admin" ? "member" : "admin" )}> 
                Yes
              </motion.button>

              <motion.button whileTap={{scale:0.75}} className='outline-none border-none text-sm px-4 py1
              rounded-md bg-gray-200 text-black hover:shadow-md' onClick={() => setIsUserRoleUpdate(false)}> 
                No
              </motion.button>
            </div>
          </motion.div>
...
const DashboardUsers = () => {
  ...
  return (
    <div className='w-full p-4 flex items-center justify-center felx-col'>
      {/*filters*/}
